...
public interface UserMissionJpaRepository extends JpaRepository<UserMission, Long> {
    ...
    @Query("""
      select new com.example.umc9th.application.mission.dto.MissionCardDto(
        ...
        case when m.endsAt is null then null
             else function('datediff', m.endsAt, current_date) end,
        case when um.status = com.example.umc9th.domain.enums.UserMissionStatus.COMPLETED
               and rv.id is null then true else false end
      )
      from UserMission um
        join um.mission m
        join m.store s
      ...
      order by
        ...
        um.assignedAt desc
    """)
    Page<MissionCardDto> findMissionCards(@Param("userId") Long userId, Pageable pageable);
  ...
  order by
    ...
    um.assignedAt desc
""")
    Page<MyMissionRowDto> findMyMissionsInRegion(
